"""KMDigits — from-scratch k-means + digit / iris helpers.

Short name: KMDigits
Drop-in stand-in for sklearn.cluster.KMeans:

    from KMDigits import KMeans
    # or
...
def purity_score(labels, y, k=None):
    _, _, p = map_clusters_to_truth(labels, y, k)
    return p


def adjusted_rand(y_true, y_pred):
    yt, yp = np.asarray(y_true, int), np.asarray(y_pred, int)
    kt, kp = int(yt.max()) + 1, int(yp.max()) + 1
