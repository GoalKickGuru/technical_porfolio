"""BankRFM — RFM + K-Means on a retail-bank transaction extract.

Short name (GitHub): BankRFM

Core-system postings → clean credits → Recency / Frequency / Monetary
→ log1p + z-score → K-Means → original-unit profiles + PCA slide.

Not a credit decision, not a BSA/AML typology, not a deposit-pricing engine.
...
def load_book(path: str = "data/bank_transactions.csv") -> pd.DataFrame:
...
def clean_postings(
    ...
    *,
    ...
    """Drop unknown CIF, non-positive qty (and optionally amount).

    Quantity > 0 removes reversals/chargebacks (TxnNo starting with R
    almost always have negative qty). Zero-amount waived fees add nothing
    ...
    if require_amount_positive:
        out = out[out["UnitAmount"] > 0]
    out["TotalSum"] = out["Quantity"] * out["UnitAmount"]
    out["TxnDate"] = pd.to_datetime(out["TxnDate"], format=date_fmt)
    out["CustomerID"] = out["CustomerID"].astype(int)
    return out.reset_index(drop=True)


def snapshot_date(sales: pd.DataFrame) -> pd.Timestamp:
    return sales["TxnDate"].max() + pd.Timedelta(days=1)
...
def build_rfm(sales: pd.DataFrame, snap: pd.Timestamp | None = None) -> pd.DataFrame:
    ...
    return (
        ...
        .agg(
            Recency=("TxnDate", lambda x: (snap - x.max()).days),
...
def name_from_medians(rfm: pd.DataFrame, labels) -> dict:
    ...
    for c, row in med.iterrows():
        if (
            ...
            names[int(c)] = "Relationship core"
        elif row["Recency"] >= med["Recency"].median() and row["Frequency"] <= med["Frequency"].median():
            names[int(c)] = "Dormant / at-risk close"
        elif row["Recency"] <= med["Recency"].median() and row["Frequency"] <= med["Frequency"].median():
            names[int(c)] = "New / light user"
        else:
            names[int(c)] = "Slipping mid-book"
    return names
...
if __name__ == "__main__":
    sales = clean_postings(load_book())
    rfm = build_rfm(sales)
    _, X, _, _ = log_scale(rfm)
    _, labels = fit_kmeans(X, k=4)
    print("customers", len(rfm), "flow", round(rfm["Monetary"].sum(), 2))
